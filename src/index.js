...
loginForm.addEventListener("submit", function(event) {
  ...
  fetch("http://127.0.0.1:3000/users", submitObj).then(function(response) {
    response.json().then(function(results) {
...
function initialFindRestaurant() {
  ...
  restaurantButton.addEventListener("click", function findingRestaurant(event) {
    ...
    /* Yelp Api get Data */
    var myurl =
      "https://cors-anywhere.herokuapp.com/https://api.yelp.com/v3/businesses/search?radius=" +
      `${distanceInput}` +
      "&open_now=" +
      `${openNowInput}` +
      "&sort_by rating=" +
            ...
            if (totalResults.length > 0) {
              ...
              for (let i = 0; i < 3; i++) {
                ...
                document
                  ...
                  .setAttribute("onclick", "findARestaurant()");
              }
            } else {
              $("#results").append(
...
function findARestaurant() {
  ...
  document
    .querySelector("#find-restaurant")
    .removeEventListener("click", findingRestaurant);
  // }
...
function saveRestaurants() {
  restaurantArray.forEach(element => {
    ...
    fetch("http://127.0.0.1:3000/restaurants", submitObj).then(function(
      response
    ) {
      response.json().then(function(results) {
...
function updateUser() {
  document.querySelector("#updates").addEventListener("submit", function(e) {
    ...
    fetch("http://127.0.0.1:3000/users/" + `${currentUserId}`, updateObj).then(
      function(response) {
